					...
					<tbody>
						{!this.props.hideHeader && (
							<tr>
								...
								{(this.props.onDeleteAction || this.props.onSetAction) && (
									<th style={{ textAlign: "left", width: ACTIONS_WIDTH, fontWeight: 200 }}>
										Initial
									</th>
								)}
							</tr>
						...
						{this.props.rows.map((row, i) => {
							return (
								<tr
									key={i}
									className={i === this.props.highlightRow ? "list-hover-background" : ""}
								>
									{row.map((r, ii) => (
										...
										<td width={ACTIONS_WIDTH}>
											...
											>
												{this.props.onSetAction && this.props.start_state === i && (
													<VSCodeButton
														appearance="icon"
														onClick={() => this.props.onSetAction?.(i)}
													>
												...
												<div style={{ marginLeft: "20px" }}>
													{this.props.onDeleteAction && (
														<VSCodeButton
															appearance="icon"
															onClick={() => this.props.onDeleteAction?.(i)}
														>
