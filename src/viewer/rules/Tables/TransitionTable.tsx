                            ...
                            <tbody>
                                ...
                                {!this.props.hideHeader &&
                                    <tr>
                                        ...
                                        {(this.props.onDeleteConditionAction) && <th style={{textAlign: 'right', width: ACTIONS_WIDTH}}>Actions</th>}
                                    </tr>
                                }
                                {/* {(transition.length >= 0) && 
                                    <tr>
                                ...
                                {(transition.length > 0) && (transition.map((condition, c_index) => {
                                    return (
                                        <tr key={c_index} className={c_index === this.props.highlightRow ? 'list-hover-background' : ''}>
                                            {(c_index === 0) && (<td width={this.props.columns[0].width}></td>)}
                                            {(c_index > 0) && (<td width={this.props.columns[0].width}>and </td>)}
                                            {condition.map((r, ii) => <td key={ii} width={this.props.columns[ii].width}>{r}</td>)}
                                            {(this.props.onDeleteConditionAction) && (
                                                <td width={ACTIONS_WIDTH}>
                                                    <div style={{textAlign: 'right'}}>
                                                        { this.props.onDeleteConditionAction &&
                                                            <VSCodeButton appearance='icon' onClick={() => this.props.onDeleteConditionAction?.(t_index, c_index)}>
                                ...
                                {(this.props.onAddConditionAction && this.props.noRowsText && transition.length === 0) && (
                                    <tr>
                                        <td colSpan={this.props.columns.length + 1} style={{textAlign: 'center'}}>
                                ...
                                {(this.props.onAddConditionAction) && (
                                    <tr>
                                        <td colSpan={this.props.columns.length + 1} style={{textAlign: 'right'}}>
                                            <VSCodeButton appearance='icon' onClick={() => this.props.onAddConditionAction?.(t_index)}>
                                ...
                                {(this.props.onAddTransitionAction) && (this.props.rows.length == 1) && (transition.length > 0) && (
                                    <tr>
                                        <td colSpan={2} style={{textAlign: 'left', marginTop:'10px'}}>
                                            <VSCodeButton style={{border:'1px', borderColor:'white', borderStyle:'solid'}} appearance='icon' onClick={() => this.props.onDeleteTransitionAction?.(t_index)}>
                                                Delete Transition
                                            </VSCodeButton>
                                        </td>
                                        <td colSpan={this.props.columns.length-1} style={{textAlign: 'right', marginTop:'10px'}}>
                                            <VSCodeButton style={{border:'1px', borderColor:'white', borderStyle:'solid'}} appearance='icon' onClick={() => this.props.onAddTransitionAction?.()}>
                                                Add Transition
                                            </VSCodeButton>
                                        </td>
                                    </tr>
