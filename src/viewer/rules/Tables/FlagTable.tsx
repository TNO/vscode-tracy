					...
					<div key={c_index}>
						<table style={{ width: "100%", borderSpacing: 0, marginBottom: "50px" }}>
							<tbody>
								...
								{condition.length > 0 &&
									condition.map((subcondition, s_index) => {
										return (
											<tr
												key={s_index}
												className={
													s_index === this.props.highlightRow ? "list-hover-background" : ""
												}
											>
												{s_index === 0 && <td width={this.props.columns[0].width}></td>}
												{s_index > 0 && <td width={this.props.columns[0].width}>and </td>}
												{subcondition.map((r, ii) => (
														...
														<div style={{ textAlign: "right" }}>
															{this.props.onDeleteSubconditionAction && (
																<VSCodeButton
																	appearance="icon"
																	onClick={() =>
																		this.props.onDeleteSubconditionAction?.(c_index, s_index)
																	}
																>
								...
								{this.props.onAddSubconditionAction && (
									<tr>
										<td colSpan={this.props.columns.length + 1} style={{ textAlign: "right" }}>
											<VSCodeButton
												appearance="icon"
												onClick={() => this.props.onAddSubconditionAction?.(c_index)}
											>
										...
										<tr>
											<td colSpan={2} style={{ textAlign: "left", marginTop: "10px" }}>
												<VSCodeButton
													...
													appearance="icon"
													onClick={() => this.props.onDeleteConditionAction?.(c_index)}
												>
													Delete Condition
												</VSCodeButton>
											</td>
											...
											>
												<VSCodeButton
													style={{ border: "1px", borderColor: "white", borderStyle: "solid" }}
													appearance="icon"
													onClick={() => this.props.onAddConditionAction?.()}
												>
													Add Condition
												</VSCodeButton>
											</td>
