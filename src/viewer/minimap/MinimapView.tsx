...
export default class MinimapView extends React.Component<Props, State> {
	...
	draw() {
		...
		const visibleItems =
			(maxVisibleItems - minVisibleItems) * Math.abs(this.state.scale - 1) + minVisibleItems;
		...
		const scrollBottom =
			minVisibleItems === maxVisibleItems
				? 0
				: Math.min(logFile.amountOfRows() * logRowHeight + scrollTop - minimapEnd, 0);
			...
			if (logFile.selectedColumnsMini[columnIndex]) {
				...
				for (let i = 0; i < colors.length; i++) {
					if (
						...
						ctx.fillRect(
							index * MINIMAP_COLUMN_WIDTH,
							counter * logRowHeight,
	...
	handleClick(e: React.MouseEvent<HTMLElement>) {
		...
		const scrollBottom =
			minVisibleItems === maxVisibleItems
				? 0
				: Math.min(logFile.amountOfRows() * logRowHeight + scrollTopBox - minimapEnd, 0);
		let nrOfRows = 0;
		let scrollTop = 0;
		if (scrollBottom < 0) {
